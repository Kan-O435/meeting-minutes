...
会議を開始すると、開いているスプレッドシート内に会議ごとのシートが一番左に作成されます。文字起こし（B5）を入力して「議事録を生成」を実行すると、Gemini APIが要約・ネクストアクションに加え、追客タイミングと先方の熱量・反応の客観的評価を生成し、同じシートへ書き込みます。
...
5. 成功するとB7へ要約、B9へネクストアクション、B13へ追客タイミング、B15へ先方の熱量・反応評価が書き込まれ、B11が「完了」になります。
6. 失敗した場合はB11が「エラー」になり、内容を説明するダイアログが表示されます。

同時に複数回実行された場合はロック（LockService）により後続の実行がスキップされます。

#### 追客タイミング（B13）・先方の熱量評価（B15）について

営業・商談的な会議を想定し、文字起こしから次の2点も抽出します。

```text
タイミング：1週間後
根拠：来週中に見積もりを送る約束をしていたため
```

```text
評価：高い
根拠：予算やスケジュールについて具体的な質問があり、次回のデモを希望していた
```

- ネクストアクションの担当者・期限と同様、**文字起こしから根拠が読み取れない場合は「不明」**とし、憶測で決めつけません。
- 熱量評価（B15）は「高い」「普通」「低い」「不明」のいずれかと、その客観的な根拠のセットです。話者の印象ではなく、質問の多さ・前向きな発言・懸念点の表明など、文字起こしに現れた具体的なやり取りを根拠にするようGeminiへ指示しています。
...
「AI議事録」→「現在の会議シートをクリア」を選択すると、確認ダイアログの後、B1（会議名）・B3（終了日時）・B5（文字起こし）・B7（要約）・B9（ネクストアクション）・B13（追客タイミング）・B15（先方の熱量・反応評価）がクリアされます。B2（開始日時）は保持されます。会議シート以外では実行できません。
