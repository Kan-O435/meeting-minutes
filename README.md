...
# 5. （音声入力を別タブで開けるようにする場合のみ・初回1回だけ）Webアプリとしてデプロイ
npx clasp deploy --description "voice-input webapp"
```
...
手順5（`clasp deploy`）は、サイドバーの「新しいタブで音声入力を開く」ボタンを使う場合にのみ必要です。一度実行すればデプロイIDが発行され、以降のコード更新は次のように同じデプロイIDへ再デプロイします（毎回`clasp deploy`するとデプロイが増え続けてしまうため）。

```bash
npx clasp deployments             # デプロイID一覧を確認
npx clasp deploy -i <デプロイID>   # 同じURLのまま最新コードを反映
```
...
サイドバーでは、ブラウザのWeb Speech API（日本語・継続認識）で音声入力ができます。「音声入力開始」で開始し、「音声入力停止」で停止すると、確定済みの文字が表示されます。内容を確認・編集し、「シートへ反映」を押すとB5へ追記されます。

#### サイドバーでマイクが許可されない場合

Apps Scriptのサイドバーは入れ子のiframe内に表示されるため、ブラウザによってはマイク許可がそもそも要求できず、「マイクの利用が許可されていません」というエラーになることがあります（Macのシステム設定やChrome全体のマイク許可が正しくても発生します。iframeのオリジンに対する許可が別問題のため）。

この場合は、サイドバー内の**「新しいタブで音声入力を開く（マイクが使えない場合）」**ボタンを押してください。同じ音声入力画面が独立したタブ（トップレベルページ）で開き、ブラウザが正しくマイク許可を要求できるようになります。認識結果は「シートへ反映」を押すと、元のスプレッドシートの該当シートへ書き込まれます。

このボタンは、Webアプリとしてのデプロイ（`npx clasp deploy`）が完了していないと使用できません。[セットアップ手順](#セットアップ手順)を参照してください。

音声入力がどうしても使えない場合は、B5へ直接入力してください（詳細は[Web Speech APIの制限](#web-speech-apiの制限)を参照）。
...
- Web Speech API（`SpeechRecognition` / `webkitSpeechRecognition`）は、Google Apps ScriptのHTMLサイドバー（入れ子のiframe内）では、ブラウザがそもそもマイク許可を要求できず「マイクの利用が許可されていません」となることがあります。これはOS・ブラウザ全体のマイク許可設定とは別に、iframeのオリジンへの許可が必要なためです。この場合はサイドバー内の「新しいタブで音声入力を開く」ボタンで、トップレベルページとして開いてください（[音声入力](#音声入力)を参照）。
- 未対応のブラウザでは、音声入力ボタンを自動的に無効化し、代替手段（B5への直接入力、OS標準の音声入力）を案内します。
...
- 「新しいタブで音声入力を開く」ボタンは、`npx clasp deploy`でWebアプリとしてデプロイされていない場合は使用できません（エラーメッセージが表示されます）。また、Webアプリを初めて開く際は、通常のメニュー実行とは別に、もう一度Googleの権限承認画面が表示されることがあります。
- Webアプリの`access`は`MYSELF`（デプロイした本人のみアクセス可）に設定しています。複数人での利用を想定する場合は`appsscript.json`の`webapp.access`を変更し、権限モデルを見直してください。
