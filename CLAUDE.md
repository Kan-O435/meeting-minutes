...
機能追加・変更を行った場合、以下を実装と一致するように更新すること。実装との記述が乖離した状態でコミットしない。

- `README.md`：セットアップ手順・APIキー管理・テスト・既知の制限・トラブルシューティング等
- `USAGE.md`：メニュー一覧・シートのレイアウト・操作手順（エンドユーザー向けの使い方）
- `ROADMAP.md`：未実装・将来検討事項の状況（実装が完了した項目は該当箇所を更新・削除する）
